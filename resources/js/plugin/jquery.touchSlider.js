...
(function(factory) {
  ...
  $.fn.touchSlider = function(settings) {
    ...
    if (opts.breakpoints) {
      ...
      for (var prop in opts.breakpoints) {
        if (prop !== "default") {
          opts.breakpoints[prop] = $.extend(
            {},
            opts.breakpoints.defaultOption,
            opts.breakpoints[prop]
          );
        }
      }
    }
  ...
  var touchSlider = {
    init: function() {
      ...
      if (this.opts.roll) {
        ...
        if (this._len % this._view > 0) {
          var blank = $(
            document.createElement(this._list.eq(0).prop("tagName"))
          )
            .hide()
            .addClass("blank");
      ...
      if (this.opts.paging) {
        ...
        for (var i = 1; i <= len; ++i) {
          paging +=
            '<span class="ts-paging-btn"><span class="ts-paging-gap">page' +
            i +
            "</span></span>";
        }
      ...
      if (this.opts.controls) {
        this._controls.append(
          '<button type="button" class="ts-prev">Prev</button><button type="button" class="ts-next">Next</button>'
        );
        this._tg.after(this._controls);
    ...
    setSize: function(page) {
      ...
      this._item_w =
        (this._width - (this._view - 1) * this.opts.gap) / this._view;

      if (page > 1 && page <= this._len) {
        page_gap =
          (page - 1) * (this._item_w * this._view + this._view * this.opts.gap);
      }
    ...
    resize: function() {
      ...
      if (this.opts.breakpoints) {
        ...
        for (var optionProp in bpCurrentOpt) {
          if (
            bpDefaultOpt.hasOwnProperty(optionProp) &&
    ...
    touchstart: function(e) {
      ...
      if (
        ...
        this._startX = e.originalEvent.touches
          ? e.originalEvent.touches[0].pageX
          : e.pageX;
        this._startY = e.originalEvent.touches
      ...
      if (
        ...
        this._left =
          (e.originalEvent.touches
            ? e.originalEvent.touches[0].pageX
            : e.pageX) - this._startX;
        this._top =
          (e.originalEvent.touches
            ? e.originalEvent.touches[0].pageY
            : e.pageY) - this._startY;
    ...
    position: function(d) {
      ...
      if (this.opts.roll) {
        ...
        if (
          ...
          (d == -1 && tmp_pos[max_chk] <= 0) ||
          (this._drag && tmp_pos[max_chk] <= 0)
        ) {
          for (var i = 0; i < view; ++i, ++p_min, ++p_max) {
        ...
        if (isTransition) {
          if (obj.speed === undefined) {
            ...
            if (obj.btn_click) {
              ...
              list_wrap_gap =
                obj.gap > 0 ? -(obj.to - obj.from) : obj.from - obj.to;
    ...
    animate: function(dir, btn_click, spd) {
      if (this._drag || !this._scroll || btn_click) {
        ...
        var gap =
          dir * (this._item_w * this._view + this._view * this.opts.gap);
        var list = this._list;
        var from = 0;
        var to = 0;
    ...
    limit_chk: function() {
      ...
      return (
        (this._start[0] === 0 && this._left > 0) ||
        (this._start[last_p] === 0 && this._left < 0)
      );
    },
